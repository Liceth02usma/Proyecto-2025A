...
class Solution:
    ...
    def __str__(self) -> str:
        ...
        self.__generar_CSV___(self.formatear_distribucion(self.distribucion_particion))
...
{self.formatear_distribucion(self.distribucion_subsistema)}
{Fore.YELLOW}Distribución de la Partición:{Style.RESET_ALL}
{self.formatear_distribucion(self.distribucion_particion)}

{Fore.YELLOW}Mejor Bi-Partición:{Style.RESET_ALL}
{Fore.MAGENTA}{self.particion}{Style.RESET_ALL}
{Fore.GREEN}φ = {self.perdida:.6f}{Style.RESET_ALL}

{Fore.CYAN}{trilinea}{Style.RESET_ALL}"""
    
    def formatear_distribucion(self, dist: np.ndarray):
        return "[" + " ".join(f"{x:.4f}" for x in dist) + "]"
    
    def __generar_CSV___(self,dist):
        pass
