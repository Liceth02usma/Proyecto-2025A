...
class QNodes(SIA):
    ...
    def __init__(self, config: Manager):
        ...
        self.memoria_emd_delta = dict()
        self.memoria_emd_union = dict()
    ...
    def algorithm(self, vertices: list[tuple[int, int]]):
        ...
        particiones = (INFTY_POS,INFTY_POS)
        i=0

        total = len(vertices_fase) - 2
        while particiones[0] !=0 and total  > i:
            ...
            print(f"Partición: {emd_particion_candidata}")
            particiones = (emd_particion_candidata,key)
            print(f"Dist: {dist_particion_candidata}")
    ...
    def funcion_submodular(
        ...
        # Llenar temporal con los nodos de delta
        ...
        # Convertimos temporal en una tupla inmutable para usarla como clave en el diccionario
        clave_delta = (tuple(sorted(temporal[0])), tuple(sorted(temporal[1])))

        # Verificamos si ya está calculado el EMD de delta
        if clave_delta in self.memoria_emd_delta:
            emd_delta, vector_delta_marginal = self.memoria_emd_delta[clave_delta]
        else:
            # Si no está en memoria, lo calculamos
            ...
            self.memoria_emd_delta[clave_delta] = (emd_delta, vector_delta_marginal)

        # Evaluación de la unión con omega
        for omega in omegas:
        ...
        # Convertimos temporal en una tupla para la clave de la unión
        clave_union = (tuple(sorted(temporal[0])), tuple(sorted(temporal[1])))

        # Verificamos si ya se calculó la unión
        if clave_union in self.memoria_emd_union:
            emd_union = self.memoria_emd_union[clave_union]
        else:
            ...
            # Guardamos solo el EMD de la unión
            self.memoria_emd_union[clave_union] = emd_union
